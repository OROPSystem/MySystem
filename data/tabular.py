...
import numpy as np
from oldutils.feature_util import get_feature_key, get_feature_names
...
class Tabular:
    ...
    def _assign_category(self):
        ...
        ) = fs.assign_category(self.get_df())
        return category_list, unique_values, default_list, frequent_values2frequency

    def _insert_data_summary(
            self, unique_values, default_list, frequent_values2frequency, sample_data_size
    ):
        df = self.get_df()
        ...
        sample_column_names = [
            "Sample {}".format(i) for i in range(1, sample_data_size + 1)
        ]
        data.columns = list(
            itertools.chain(
                [
    ...
    def get_new_features(self, form):
        ...
        for k, v in self.get_defaults().items():
            if k not in fs_list:
                new_features[k] = (
                    form[k] if k not in self.get_targets() else self.get_defaults()[k]
                )
        return new_features
    ...
    def load_features(self):
        ...
        self.set_data_sumary(
            self._insert_data_summary(
                unique_values,
                default_list,
                frequent_values2frequency,
        ...
        self.set_defaults(
            dict(zip(self.get_data_summary().index.tolist(), default_values))
        )
        ...
        if len(targets) == 1:
            ...
            if target_type == "range":
                ...
                for categ, feature in zip(categories, df.columns):
                    new_categ_list.append(
                        categ if feature != targets[0] else "categorical"
                    )
                self.set_categories(new_categ_list)
                summary_data.Category = new_categ_list
                fs.update(
                    categories,
                    dict(zip(summary_data.index.tolist(), summary_data.Defaults)),
                )
    ...
    def update_feature_columns(self):
        ...
        self.set_feature_columns(
            fs.create_tf_features(
                categories, targets, self.get_normalize(), training_path
            )
        )
        self.set_all_feature_columns(
            fs.create_tf_features(
                categories,
                targets,
                self.get_normalize(),
    ...
    def update_features(self, cat_columns, default_values):
        ...
        self.get_feature_selection().update(
            cat_columns, dict(zip(summary_data.index.tolist(), default_values))
        )

        column_categories = {}
        for label, old_cat in zip(summary_data.index, old_categories):
            new_cat = summary_data.Category[
                label
            ]  # if data.Category[label] != 'range' else 'int-range'
            cat = (
                new_cat + "-" + old_cat.replace("none-", "")
                if "none" in new_cat
                else new_cat
            )
    ...
    def split_dataset(self, percent=None):
        ...
        train_file = os.path.join(file.rstrip(basename), "train_orgin", basename)
        ...
        train_df, val_df = train_test_split(
            df, test_size=val_frac, stratify=stratify, random_state=42
        )
        ...
        if percent[2] != 0:
            ...
            train_df, test_df = train_test_split(
                train_df, test_size=test_size, stratify=stratify, random_state=42
            )
    ...
    def get_num_inputs(self):
        fs_list = self.get_feature_selection().group_by(self.get_categories())["none"]
        filtered = [
            f for f in self.get_feature_columns() if get_feature_key(f) not in fs_list
        ]
    ...
    def get_target_labels(self):
        ...
        if target_type == "categorical" or target_type == "hash":
            ...
            return [
                str(a)
                for a in list(
                    range(min(fs.df[target].values), max(fs.df[target].values))
                )
            ]
        return None
        ...
        for c in df.columns:
            if c in features.keys():
                if df[c].dtype == "object":
                    ...
                    mapp = {
                        y: x for x, y in dict(enumerate(df[c].cat.categories)).items()
                    }
                    features[c] = float(mapp[features[c]])
                else:
                    features[c] = float(features[c])
        feats = (
            df[self.get_feature_names()]
                .append(
                pd.DataFrame(pd.Series(features)).transpose()[self.get_feature_names()]
            )
                .tail(1)
        )
        ...
        for c in df.columns:
            if c in features.keys():
                if df[c].dtype == "object":
                    df.loc[:, c] = df.loc[:, c].astype("category")
                    mapp = {
                        x: y for x, y in dict(enumerate(df[c].cat.categories)).items()
                    }
    ...
    def clean_values(self, df):
        ...
        for c in df.columns.values:
            df.loc[:, c] = (
                df.loc[:, c]
                    .fillna(self._keyed_defaults[c])
                    .astype(type(self._keyed_defaults[c]))
            )
        return df
    ...
    def get_categorical_features(self):
        ...
        categorical_index = [
            list(df.columns.values).index(x) for x in categorical_features
        ]
    ...
    def train_input_fn(self, batch_size, num_epochs):
        csv_dataset = make_csv_dataset(
            [self.get_train_file()],
            batch_size=batch_size,
            shuffle=True,
            label_names=self.get_targets(),
    ...
    def input_predict_fn(self, features):
        ...
        for t in self.get_targets():
            del features[t]
        features = {
            k: features[k] for k in get_feature_names(self.get_feature_columns())
        }
        for k, v in features.items():
            features[k] = (
                np.array([v]).astype(df[k].dtype)
                if df[k].dtype == "object"
                else np.array([float(v)]).astype(df[k].dtype)
            )
        return tf.estimator.inputs.numpy_input_fn(
            x=features, y=None, num_epochs=1, shuffle=False
        )
    ...
    def serving_input_receiver_fn(self):
        ...
        receiver_tensors = {
            k: tf.placeholder(v.dtype, [None, 1]) for k, v in feature_spec.items()
        }
        return tf.estimator.export.ServingInputReceiver(
            receiver_tensors=receiver_tensors, features=receiver_tensors
        )

    def get_all_test_files(self):
        return [
            f
            for f in os.listdir(os.path.join(self.get_base_path(), "test"))
            if not f.startswith(".")
        ]
