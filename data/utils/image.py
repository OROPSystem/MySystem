...
from oldutils.preprocessing import has_header
...
def random_central_crop(image, minval, maxval):
    with ops.name_scope(None, "central_crop", [image]):
        ...
        def _get_dim(tensor, idx):
            ...
            return ops.array_ops.shape(tensor)[idx], True
        ...
        central_fraction = tf.random_uniform(
            [], minval=minval, maxval=maxval, dtype=tf.float64
        )
        ...
        img_hd = ops.math_ops.to_double(img_h)
        bbox_h_start = ops.math_ops.to_int32((img_hd - img_hd * central_fraction) / 2)

        img_wd = ops.math_ops.to_double(img_w)
        bbox_w_start = ops.math_ops.to_int32((img_wd - img_wd * central_fraction) / 2)

        bbox_h_size = img_h - bbox_h_start * 2
        bbox_w_size = img_w - bbox_w_start * 2

        if rank == 3:
            bbox_begin = ops.array_ops.stack([bbox_h_start, bbox_w_start, 0])
            bbox_size = ops.array_ops.stack([bbox_h_size, bbox_w_size, -1])
        else:
            bbox_begin = ops.array_ops.stack([0, bbox_h_start, bbox_w_start, 0])
            bbox_size = ops.array_ops.stack([-1, bbox_h_size, bbox_w_size, -1])

        image = ops.array_ops.slice(image, bbox_begin, bbox_size)
...
def find_images_test_file(path):
    for f in os.listdir(path):
        if os.path.isfile(os.path.join(path, f)):
            if os.path.splitext(os.path.join(path, f))[1] not in [
                ".jpg",
                ".jpeg",
                ".png",
                ".PNG",
