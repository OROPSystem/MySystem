...
def imresize(arr, size, interp='bilinear', mode=None):
    ...
    im_new = im.resize(size, resample=func[interp])  # 调用PIL库中的resize函数
...
class Image:
    def __init__(self, dataset_path, test_path, mode, name):
        ...
        self._train_images = (
            ...
            self._test_images
        ) = self._train_labels = self._val_labels = self._test_labels = None
    ...
    def _read_data(self):
        if self.get_mode() == 1:
            (
                ...
                self._labels,
                self._class_names,
            ) = find_image_files_folder_per_class(self.get_dataset_path())
        elif self.get_mode() == 2:
            info_file = [
                f
                for f in os.listdir(self.get_dataset_path())
                if f.startswith("labels.")
            ]
            self._images, self._labels, self._class_names = find_image_files_from_file(
                self.get_dataset_path(),
                os.path.join(self.get_dataset_path(), info_file[0]),
            )
        elif self.get_mode() == 3:
            npz_file = [
                f for f in os.listdir(self.get_dataset_path()) if f.endswith(".npz")
            ]
    ...
    def _parse_augmentation_options(self, image, label):
        ...
        if "saturation" in options:
            image = tf.image.random_saturation(
                image, float(params["from_saturation"]), float(params["to_saturation"])
            )
        if "contrast" in options:
            image = tf.image.random_contrast(
                image, float(params["from_contrast"]), float(params["to_contrast"])
            )
        if "brightness" in options:
            image = tf.image.random_brightness(
                image, float(params["max_delta_brightness"])
            )
        ...
        if "quality" in options:
            image = tf.image.random_jpeg_quality(
                image, int(params["from_quality"]), int(params["to_quality"])
            )
    ...
    def train_input_fn(self, batch_size, num_epochs):
        ...
        dataset = (
            dataset.shuffle(len(self._train_images))
                .repeat(num_epochs)
                .map(self._parse_function)
                .map(self._parse_augmentation_options)
                .map(self._norm_function)
                .batch(batch_size)
        )
    ...
    def validation_input_fn(self, batch_size):
        ...
        dataset = (
            dataset.map(self._parse_function).map(self._norm_function).batch(batch_size)
        )
        dataset = dataset.prefetch(1)
        return dataset
    ...
    def input_predict_fn(self, image):
        if len(image.shape) == 2:
            image = imresize(
                image, self.get_image_size()[0:2], interp="bilinear"
            ).reshape(self.get_image_size())
        else:
    ...
    def get_all_test_files(self):
        test_path = self.get_dataset_path().replace("train_orgin", "test")
        try:
            return [
                name
