...
import pandas as pd
import numpy as np
from ..oldutils.sys_ops import mkdir_recursive, tree_remove
import itertools
import functools
import os

with open("system/generator/cluster", "r") as f:
    text = f.read()

with open("system/generator/grammar", "r") as f:
    grammar = f.read()
...
class DataSetGeneratorTransformer(Transformer):
    ...
    def random_variables(self, names, values):
        assert len(names) == values.shape[1], (
            f"Value dimension do not agree with variable dimension {names} have "
    ...
    def range(self, lower, upper):
        assert (
                upper >= lower
        ), f"Range lower is larger than upper limit {lower} > {upper}"
    ...
    def combine(self, left, right):
        ...
        result = np.array([left_or_right(l, r) for l, r in zip(left, right)]).reshape(
            (-1, 1)
        )
        return result
    ...
    def addlist(self, name, low, high):
        return np.sum(
            [self.columns[name + str(i)] for i in range(low, high + 1)], axis=0
        )
