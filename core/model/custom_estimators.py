...
def dic_initializer_param(initializer, params):
    for key, value in params.items():
        params[key] = (
            float(value) if initializers[initializer][key]["type"] == "float" else value
        )
    return params
...
def get_label_ids(labels, label_vocabulary):
    if label_vocabulary is None:
        if not labels.dtype.is_integer:
            raise ValueError(
                "Labels dtype should be integer. Instead got {}.".format(labels.dtype)
            )
        label_ids = labels
    else:
        label_ids = lookup_ops.index_table_from_tensor(
            vocabulary_list=tuple(label_vocabulary), name="class_id_lookup"
        ).lookup(labels)
    return label_ids
...
def create_output(features, params, mode):
    ...
    label_vocabulary = (
        params["label_vocabulary"] if "label_vocabulary" in params else None
    )
    ...
    if params["mode"] == "custom":
        ...
        model = keras_tfjs_loader.load_keras_model(
            params["model_path"], load_weights=False, use_unique_name_scope=False
        )
...
def classifier(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        predictions = {
            ...
            if "softmax" not in output.name.lower()
            else output,
            "logits": output,
    ...
    reshaped_labels = (
        reshaped_labels
        if "sparse" in params["loss_function"]
        else tf.one_hot(tf.reshape(label_ids, [-1]), params["n_classes"])
    )
...
def binary_classifier(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
        predictions = {
            ...
            if "sigmoid" not in output.name.lower()
            else output,
            "logits": output,
            "classes": get_label_classes(predicted_classes, label_vocabulary),
        }
...
def linear(features, feature_columns, params):
    return _linear(
        get_num_outputs(params), features, feature_columns, params["sparse_combiner"]
    )
...
def dnn(net, params, mode):
    ...
    if "params" in params["kernel_initializer"]:
        dict_param_initializer = dic_initializer_param(
            initializer_name, params["kernel_initializer"]["params"]
        )

        kernel_initializer = getattr(
            tf.initializers, initializers_opts[initializer_name]
        )(**dict_param_initializer)
    model = _DNNModel(
        get_num_outputs(params),
        params["hidden_units"],
        params["activation_fn"],
        dropout,
        params["batch_norm"],
        kernel_regularizer_params=regularizer_params,
        ...
        for node in nodes:
            ...
            if len(computed_data) == len(reference_input_tensors):
                ...
                with K.name_scope(layer.name):
                    ...
                    if (
                        hasattr(layer, "activity_regularizer")
