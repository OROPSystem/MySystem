...
class Session:
    ...
    def get_metric(self):
        return (
            "Accuracy"
            if self.get_helper().get_mode() == "classification"
            else "R-squared"
        )
    ...
    def set_generate_df(self, dataset_name, USER_ROOT):
        path = os.path.join(
            USER_ROOT, session["user"], "datasets", dataset_name, dataset_name + ".csv"
        )
        df = pd.read_csv(path)
    ...
    def write_params(self):
        ...
        data_path = os.path.join(
            os.path.dirname(self.get_config_file()), hlp.get_dataset_name() + ".pkl"
        )
        ...
        if self.get("mode") == "custom":
            self.get_writer().add_item(
                "CUSTOM_MODEL", "custom_path", self.get("custom_path")
            )
            self.get_writer().add_item(
                "CUSTOM_MODEL", "transform_path", self.get("transform_path")
            )
            self.get_writer().add_item(
                "CUSTOM_MODEL", "loss_function", self.get("loss")
            )
