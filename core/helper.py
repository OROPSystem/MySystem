...
from oldutils.explain_util import get_reg_explain, get_class_explain, clean_predict_table
from oldutils.request_util import *
from oldutils.sys_ops import unzip, tree_remove, find_dataset_from_numpy
...
class Tabular(Helper):
    ...
    def process_features_request(self, request):
        ...
        cat_columns, default_values = feature_util.reorder_request(
            get_default_feature(request),
    ...
    def process_targets_request(self, request):
        ...
        if not preprocessing.check_train(
                self._dataset.get_train_file(), self._dataset.get_targets()
        ):
            return {
                "error": "Number of classes for the target should be greater than 1."
            }

        num_outputs = self.get_num_outputs()
        input_shape = self.get_input_shape()
        hidden_layers = param_utils.get_hidden_layers(
            input_shape, num_outputs, self._dataset.get_train_size()
        )
        result = {
            "error": False,
    ...
    def get_default_data_example(self):
        dict_types, categoricals = run_utils.get_dictionaries(
            self._dataset.get_defaults(),
            self._dataset.get_categories(),
            self._dataset.get_feature_selection(),
            self._dataset.get_targets(),
        )
        sfeatures = feature_util.remove_targets(
            self._dataset.get_defaults(), self._dataset.get_targets()
        )
        explain_disabled = run_utils.get_explain_disabled(
            self._dataset.get_categories()
        )
    ...
    def get_new_features(self, request, default_features=False):
        return (
            self._dataset.get_new_features(request.form)
            if not default_features
            else self._dataset.get_defaults()
        )
    ...
    def process_explain_request(self, request):
        ...
        input_check = explain_util.check_input(
            num_feat,
            top_labels,
            len(new_features),
    ...
    def generate_rest_call(self, pred):
        call, d, epred = sys_ops.gen_example(
            self._dataset.get_targets(),
            self._dataset.get_data_summary(),
    ...
    def create_ice_data(self, request):
        file_path, unique_val_column = explain_util.generate_ice_df(
            request,
            self._dataset.get_df(),
    ...
    def process_ice_request(self, request, unique_val_column, pred):
        ...
        lab, probs = explain_util.get_exp_target_prediction(
            self._dataset.get_targets(), exp_target, pred, self._dataset.get_dtypes()
        )
        data = {
            exp_feature: unique_val_column,
            exp_target: lab,
    ...
    def test_upload(self, request):
        ...
        test_filename = os.path.join(
            self._dataset.get_base_path(), "test", test_file.filename
        )
    ...
    def test_request(self, request):
        if "filename" in request.get_json():
            test_filename = os.path.join(
                self._dataset.get_base_path(), "test", get_filename(request)
            )
        else:
            test_filename = (
                self._dataset.get_test_file()[0]
                if isinstance(self._dataset.get_test_file(), list)
    ...
    def process_test_predict(self, df, final_pred, test_filename):
        return sys_ops.save_results(
            df,
            final_pred["preds"],
            self._dataset.get_targets(),
            ntpath.basename(test_filename),
...
class Image(Helper):
    ...
    def get_data(self):
        ...
        return {
            "height": self._dataset.get_sample().shape[0],
            "width": self._dataset.get_sample().shape[1],
            "data": self.get_labels_images(),
            "num_outputs": self._dataset.get_num_outputs(),
    ...
    def get_labels_images(self):
        ...
        labels = (
            ...
            if isinstance(self._dataset.get_labels(), list)
            else self._dataset.get_labels().tolist()
        )
    ...
    def process_features_request(self, request):
        ...
        self._dataset.set_image_size(
            request.get_json()["height"], request.get_json()["width"], c
        )
    ...
    def get_default_data_example(self):
        # TODO
        example = self._dataset._val_images[
            np.random.choice(np.arange(len(self._dataset._val_images)))
        ]
    ...
    def process_test_predict(self, df, final_pred, test_filenames):
        ...
        return sys_ops.save_image_results(
            df,
            final_pred["preds"],
            self._dataset.get_targets(),
            test_filenames,
            self._dataset.get_dataset_path().replace("train_orgin", ""),
        )
        ...
        if "filename" in request.get_json():
            if get_filename(request) == "TEST FROM SPLIT":
                ...
                test_path = os.path.join(
                    self._dataset.get_dataset_path().replace("train_orgin", "test"),
                    get_filename(request),
                )
                option = [f for f in os.listdir(test_path) if f.startswith(".option")][
                    0
                ]
                ...
                if option == ".option0":
                    ...
                    all_classes = [
                        c for c in os.listdir(test_path) if not c.startswith(".")
                    ]
    ...
    def test_upload(self, request):
        ...
        try:
            if path_file.endswith(".npz"):
                try:
                    _, test_data = find_dataset_from_numpy(
                        path_file, requires_y=False, only_test=True
                    )
                ...
                if find_images_test_file(dataset_test_path):
                    open(
                        os.path.join(dataset_test_path, ".option1"), "w"
                    )  # ONLY IMAGES
                else:
                    try:
                        ...
                        open(
                            os.path.join(dataset_test_path, ".option2"), "w"
                        )  # FOLDER PER CLASS
                    except AssertionError:
                        try:
                            info_file = [
                                f
                                for f in os.listdir(dataset_test_path)
                                if f.startswith("test.") or f.startswith("labels.")
                            ]
                            assert len(info_file) == 1
                            info_path = os.path.join(dataset_test_path, info_file[0])
                            f, n, c = find_image_files_from_file(
                                dataset_test_path, info_path, require_all=False
                            )
                            assert len(c) == len(self.get_target_labels())
                            open(
                                os.path.join(dataset_test_path, ".option3"), "w"
                            )  # LABELS.TXT
                            os.rename(
                                info_path, os.path.join(dataset_test_path, "labels.txt")
                            )
    ...
    def explain_return(self, request, results):
        ...
        temp, mask = result.get_image_and_mask(
