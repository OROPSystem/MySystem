...
def get_streamlit_params():
    ...
    if library == "PyTorch":
        ...
        loads = st.sidebar.selectbox(label="Load", options=os.listdir(load_range))
        params['load'] = loads
        ...
        no_test = st.sidebar.text_input(label='Number per class for test', value='300')
        params['no_test'] = int(no_test)
        
        # Signal Size
        signal_size = st.sidebar.text_input(label='Signal Size', value='1200')
