...
def get_class_explain(result, MAX_FEATURES=10):
    ...
    for i in range(0, len(result.top_labels)):
        k = (
            "NOT "
            + result.class_names[result.top_labels[i]]
            + " |  "
            + result.class_names[result.top_labels[i]]
        )
        graphs[k] = {}
...
def clean_predict_table(predict_table, MAX_FEATURES=10):
    ...
    if len(predict_table["columns"]) > MAX_FEATURES:
        ...
        new_predict_table["data"].append(
            1 - np.sum(np.array(new_predict_table["data"]))
        )
    return new_predict_table


def check_input(num_feat, top_labels, max_feat, max_labels):
    if not num_feat or int(num_feat) < 1 or int(num_feat) > max_feat:
        return "Wrong number of features. Please try again."

    if not top_labels or int(top_labels) < 1 or int(top_labels) > max_labels:
...
def get_exp_target_prediction(targets, exp_target, final_pred, dtypes):
    index = (
        targets.index(exp_target) if len(targets) > 1 else None
    )  # Target index in targets
    # TODO
    if index is not None:
        if exp_target in dtypes["numerical"]:
            return [float(x[index]) for x in final_pred["preds"]], None
        return final_pred["preds"][index], [
            [float(score) for score in scores[index]] for scores in final_pred["scores"]
        ]

    if exp_target in dtypes["numerical"]:
        return [float(x) for x in final_pred["preds"]], None
    return final_pred["preds"], [
        [float(score) for score in scores] for scores in final_pred["scores"]
    ]
