...
from database import UserSessionModel
...
def get_db_user(username):
    return db.session.query(UserModel.id).filter_by(username=username).scalar()
...
def sign_up(form):
    ...
    if (
            db.session.query(UserModel.id).filter_by(username=form.username.data).scalar()
            is not None
    ):
        return False
    new_user = UserModel(
...
def get_usersession_by_username(username):
    return UserSessionModel.query.filter_by(username=username).first()


def check_user_has_token(username):
    if db.session.query(UserSessionModel.id).filter_by(username=username).scalar() is None:
        create_user_session(username)
    else:
        timestamp = (
            db.session.query(UserSessionModel.timestamp)
                .filter_by(username=username)
                .scalar()
        )
        if (
                datetime.now() - timestamp
        ).total_seconds() > EXPIRE_TIME or timestamp is None:
            # TOKEN EXPIRED
...
def create_user_session(username):
    token = randomStringwithDigit(32)
    new_usersession = UserSessionModel(
        username=username, token=token, timestamp=datetime.now()
    )
...
def get_token_user(username):
    check_user_has_token(username)
    token = db.session.query(UserSessionModel.token).filter_by(username=username).scalar()
    return token
