...
def set_form(form, CONFIG_FILE):
    if os.path.isfile(CONFIG_FILE):
        ...
        if "EXPERIMENT" in reader.keys():
            form.experiment.form.keep_checkpoint_max.default = reader["EXPERIMENT"][
                "keep_checkpoint_max"
            ]
            form.experiment.form.save_checkpoints_steps.default = reader["EXPERIMENT"][
...
def get_params(config_file, appConfig):
    ...
    if os.path.isfile(config_file):
        ...
        if "EXPERIMENT" in reader.keys():
            ...
            dict["save_checkpoints_steps"] = reader["EXPERIMENT"][
                "save_checkpoints_steps"
            ]
...
def set_checkpoint_dir(all_params_config, checkpoint):
    all_params_config.set(
        "PATHS",
        "checkpoint_dir",
        os.path.join(all_params_config.export_dir(), checkpoint),
    )
