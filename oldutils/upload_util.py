...
def get_text(file_name):
    file = open(os.path.join("generator", file_name))
    text = file.read()
    file.close()
    return text
...
def new_config(dataset_name, username, sess, USER_ROOT, appConfig):
    ...
    files = [
        ...
        if f in [".tabular", ".images1", ".images2", ".images3"]
    ]
    ...
    if files[0].startswith(".tabular"):
        ...
        dataset = Tabular(
            dataset_name, os.path.join(dataset_path, dataset_name + ".csv")
        )
        path_test = os.path.join(
            get_dataset_path(USER_ROOT, username, dataset_name), "test"
        )
        test_files = [
            os.path.join(path_test, f)
        ...
        test_path = (
            ...
            if len(os.listdir(os.path.join(dataset_path, "test"))) > 0
            else None
        )
