...
def find_dataset_from_numpy(path_file, requires_y=True, only_test=False):
    ...
    if not only_test:
        if "y_train" in data:
            data["x"] = (
                data.pop("x_train") if "x_train" in data else data.pop("X_train")
            )
...
def unzip(path_to_zip_file, directory_to_extract_to):
    try:
        ...
        dirs = [
            ...
            if os.path.isdir(os.path.join(directory_to_extract_to, f))
        ]
...
def delete_recursive(paths, export_dir):
    if os.path.isdir(export_dir):
        for p in paths:
            if os.path.exists(os.path.join(export_dir, p)):
...
def save_image_results(test_labels, result, targets, filenames, base_path):
    ...
    base_path = base_path.replace("train_orgin", "predictions")
...
def gen_example(targets, data, df, model_name, pred):
    ...
    for i in range(len(feat_keys)):
        if df[feat_keys[i]].dtype != "object":
            example[feat_keys[i]] = (
                np.array([float(data["Defaults"][feat_keys[i]])])
                    .astype(dtypes[i])
                    .tolist()
            )
        else:
    ...
    call += 'MODEL_NAME="..."\n'
    call += (
        "curl -X POST http://${DOCKER_HOST}:8501/v1/models/${MODEL_NAME}/versions/1"
        ":predict -d "
    )
...
def load_cy_model(model, user, user_root):
    custom_path = os.path.join(
        user_root, user, "models", model, "custom", "model_cy.json"
    )
    cy_model = None
...
def load_cy_input(model, user, user_root):
    custom_path = os.path.join(
        user_root, user, "models", model, "custom", "input_model_cy.json"
    )
    if os.path.isfile(custom_path):
        input_cy_model = json.load(open(custom_path), object_pairs_hook=OrderedDict)
        return (
            input_cy_model["dataset_params"],
            input_cy_model["data"],
            input_cy_model["num_outputs"],
        )
    return {}, {}, {}
...
def get_canned_json(USER_ROOT, username, model_name):
    return os.path.join(
        USER_ROOT, username, "models", model_name, "custom", "canned_data.json"
    )


def get_log_path(USER_ROOT, username, model_name):
    return os.path.join(
        USER_ROOT, username, "models", model_name, "log", "tensorflow.log"
    )
...
def create_split_folders(main_path):
    os.makedirs(os.path.join(main_path, "train_orgin"), exist_ok=True)
...
def delete_file_test(request, param_configs, USER_ROOT, username):
    ...
    path = os.path.join(
        get_dataset_path(USER_ROOT, username, dataset_name), "test", filename
    )
    try:
